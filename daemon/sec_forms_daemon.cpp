...
int ServiceThread(OurParams& our_params)
{
	...
	<< "****************************************************" << "\n"
	<< "***  Welcome to The SEC Forms Daemon, Mr. Bond! ***" << "\n"
	<< "****************************************************" << endl;
	...
	// i_download_sleep_time defaults to JDA::FormsCommon::DEFAULT_DOWNLOAD_SLEEP_TIME if parsing fails...
	int i_download_sleep_time = JDA::Utils::stringToInt( configMap["download_sleep_time"], JDA::FormsCommon::DEFAULT_DOWNLOAD_SLEEP_TIME );
	...
	else if( our_params.s_manual_denormalize_form_accession_number.length() > 0 ){
...
void getConfig( map<string, string>& configMap, OurParams& our_params ) {
	...
	* in setupLogger()...
	*/
	configMap["debug_level"] = JDA::FormsCommon::DEFAULT_DEBUG_LEVEL;
	configMap["debug_log_file_path"] = JDA::FormsCommon::DEFAULT_DEBUG_LOG_FILE_PATH;
	configMap["debug_log_file_on"] = JDA::Utils::boolToString( JDA::FormsCommon::DEFAULT_DEBUG_LOG_FILE_ON );
	configMap["debug_log_file_append"] = JDA::Utils::boolToString( JDA::FormsCommon::DEFAULT_DEBUG_LOG_FILE_APPEND ); 
	configMap["debug_stdout_on"] = JDA::Utils::boolToString( JDA::FormsCommon::DEFAULT_DEBUG_STDOUT_ON );

	configMap["load_daily_indexes"] = JDA::Utils::boolToString( JDA::FormsCommon::DEFAULT_LOAD_DAILY_INDEXES );
	configMap["load_next_edgar_filing_header"] = JDA::Utils::boolToString( JDA::FormsCommon::DEFAULT_LOAD_NEXT_EDGAR_FILING_HEADER );

	configMap["download_sleep_time"] = JDA::Utils::intToString( JDA::FormsCommon::DEFAULT_DOWNLOAD_SLEEP_TIME );
...
void setupLogger(
	...
	kenny_loggins.setDebugLevel( JDA::FormsCommon::DEFAULT_DEBUG_LEVEL );
...
//
//			JDA::FormsUtils::DownloadWindowType downloadWindowType = in_which_download_window( evening_start_hour, evening_end_hour, hour, day_of_week );
//

	...
	*/
	...
	int JDA::MongoDbClient::command( const string& s_db_name, const string& s_collection_name, const string& s_json_command, JDA::MongoDbClient::IMongoDbClientCallback* p_callback ){
		...
		while (mongoc_cursor_next (p_cursor, &p_bson_doc)) {
			...
			if( m_p_logger ){
				(*m_p_logger)(JDA::Logger::INFO) << sWho << "(): " << "cursor[" << row_number << "]: json_string = " << this->bson_as_json_string( p_bson_doc ) << "\n" << endl; 
			}

			if( m_p_logger ){
				(*m_p_logger)(JDA::Logger::INFO) << sWho << "(): " << "cursor[" << row_number << "]: pretty_json_string =...\n" << endl;
				(*m_p_logger)(JDA::Logger::INFO) << sWho << "(): " << "cursor[" << row_number << "]: pretty_json_string =\n" << bsonTraverser.bson_as_pretty_json_string( p_bson_doc ) << "\n" << endl; 
			}
	...
	void JDA::MongoDbClient::lazyInitPMongocClient(){
		...
		if( m_p_client == NULL ){
			...
			if( m_p_logger != NULL ){
				(*m_p_logger)(JDA::Logger::DEBUG) << sWho << "(): " << "Calling mongoc_client_new( m_s_uri_str = \"" << m_s_uri_str << "\" )..." << endl;
			}
